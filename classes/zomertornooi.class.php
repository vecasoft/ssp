...
class SSP_zomertornooi { // define the class
    ...
    static function GetAantalPlaatsen($pCategorie) {
        ...
        $taRec = SSP_db::Get_SX_taRec('TORNOOI_CATEGORIE',$pCategorie);
        ...
        $sqlStat = "Select count(*) as aantal from ssp_zomertornooi where categorie = '$pCategorie'";
        $db->Query($sqlStat);
    ...
    static function ChkCategorieVolzet() {
        ...
        $sqlStat = "Select * from sx_ta_tables where taTable = 'TORNOOI_CATEGORIE'";
        ...
        while ($taRec = $db->Row() ){
            ...
            if ($aantalPlaatsen <= 0) {
                ...
                $sqlStat = "Update sx_ta_tables set taRecStatus = 'H' where taTable = 'TORNOOI_CATEGORIE' and taCode = '$categorie' ";
                $db2->Query($sqlStat);

            } else {

                $sqlStat = "Update sx_ta_tables set taRecStatus = 'A' where taTable = 'TORNOOI_CATEGORIE' and taCode = '$categorie' ";
                $db2->Query($sqlStat);
    ...
    static function GetAantalPlaatsenHTML() {
        ...
        $sqlStat = "Select * from sx_ta_tables where taTable = 'TORNOOI_CATEGORIE' order by taSort";
