...
{ // define the class
    ...
    static function GetAnalytischeStructuurJSON(){
        ...
        foreach ($struct as $detail){
            ...
            $budget2021 = $detail['budget_2021'] + 0;
    ...
    static function GetAnalytischeStructuurArray(){
        ...
        while ($arRec = $db->Row()) {
            ...
            // Budget (vorig jaar)
            $sqlStat = "Select * from efin_ab_analytische_rekening_budgetten where abAnalytischeRekening = $analytischeRekening and abPeriode = '2021'";
            $db2->Query($sqlStat);

            if ($abRec = $db2->Row()){

                if ($abRec->abLinkType == '*SPECIFIEK')
                    $budget2021 = $abRec->abBudget;
            }

            $detail['budget_2021'] = $budget2021;
